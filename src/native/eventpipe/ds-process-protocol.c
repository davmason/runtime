...
process_info_3_payload_get_size (DiagnosticsProcessInfo3Payload *payload)
{
	...
	size += sizeof(uint32_t);
	size += (payload->runtime_rid != NULL) ?
		(ep_rt_utf16_string_len (payload->runtime_rid) + 1) * sizeof(ep_char16_t) : 0;
	...
	uint16_t *size)
{
	...
	*size -= sizeof (process_info->runtime_cookie);
	...
	// LPCWSTR runtime_rid;
	if (success)
		success &= ds_ipc_message_try_write_string_utf16_t (buffer, size, process_info->runtime_rid);
	...
	const ep_char16_t *runtime_rid)
{
	...
	payload->managed_entrypoint_assembly_name = managed_entrypoint_assembly_name;
	payload->clr_product_version = clr_product_version;
	payload->runtime_rid = runtime_rid;
	...
	DiagnosticsIpcStream *stream)
{
	...
	runtime_rid = ep_rt_utf8_to_utf16le_string (ep_rt_runtime_rid_get_utf8 (), -1);
	ep_raise_error_if_nok (runtime_rid != NULL);
