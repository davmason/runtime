...
// _Requires_lock_held (ep)
static uint32_t generate_session_index(void);

// _Requires_lock_held (ep)
static bool is_session_id_in_collection(EventPipeSessionID id);
...
static void log_process_info_event(EventPipeEventSource *event_source);

// _Requires_lock_held (ep)
static void disable_holding_lock(
    EventPipeSessionID id,
    EventPipeProviderCallbackDataQueue *provider_callback_data_queue);

static void disable_helper(EventPipeSessionID id);
...
static const ep_char8_t *get_next_config_value(
    const ep_char8_t *data,
    const ep_char8_t **start,
    const ep_char8_t **end);

static ep_char8_t *get_next_config_value_as_utf8_string(const ep_char8_t **data);

static uint64_t get_next_config_value_as_uint64_t(const ep_char8_t **data);

static uint32_t get_next_config_value_as_uint32_t(const ep_char8_t **data);

static void enable_default_session_via_env_variables(void);

static bool session_requested_sampling(EventPipeSession *session);

static bool ipc_stream_factory_any_suspended_ports(void);
...
EventFilterDescriptor *ep_event_filter_desc_alloc(uint64_t ptr, uint32_t size, uint32_t type)
{
...
EventFilterDescriptor *ep_event_filter_desc_init(
...
void ep_event_filter_desc_fini(EventFilterDescriptor *filter_desc)
{
    ;
}

void ep_event_filter_desc_free(EventFilterDescriptor *filter_desc)
{
...
EventPipeProviderCallbackDataQueue *ep_provider_callback_data_queue_init(
    EventPipeProviderCallbackDataQueue *provider_callback_data_queue)
{
    EP_ASSERT(provider_callback_data_queue != NULL);
    ep_rt_provider_callback_data_queue_alloc(&provider_callback_data_queue->queue);
    return ep_rt_provider_callback_data_queue_is_valid(&provider_callback_data_queue->queue)
               ? provider_callback_data_queue
               : NULL;
}

void ep_provider_callback_data_queue_fini(EventPipeProviderCallbackDataQueue *provider_callback_data_queue)
{
...
EventPipeProviderCallbackData *ep_provider_callback_data_alloc(
...
EventPipeProviderCallbackData *ep_provider_callback_data_alloc_move(
    EventPipeProviderCallbackData *provider_callback_data_src)
{
    EventPipeProviderCallbackData *instance = ep_rt_object_alloc(EventPipeProviderCallbackData);
    ep_raise_error_if_nok(instance != NULL);
...
EventPipeProviderCallbackData *ep_provider_callback_data_init(
...
EventPipeProviderCallbackData *ep_provider_callback_data_init_move(
    EventPipeProviderCallbackData *provider_callback_data_dst,
    EventPipeProviderCallbackData *provider_callback_data_src)
{
    EP_ASSERT(provider_callback_data_dst != NULL);
...
void ep_provider_callback_data_fini(EventPipeProviderCallbackData *provider_callback_data)
{
    ep_return_void_if_nok(provider_callback_data != NULL);
    ep_rt_utf8_string_free(provider_callback_data->filter_data);
}

void ep_provider_callback_data_free(EventPipeProviderCallbackData *provider_callback_data)
{
    ep_return_void_if_nok(provider_callback_data != NULL);
...
EventPipeProviderConfiguration *ep_provider_config_init(
...
void ep_provider_config_fini(EventPipeProviderConfiguration *provider_config)
{
    ;
}
...
EventPipeExecutionCheckpoint *ep_execution_checkpoint_alloc(const ep_char8_t *name, ep_timestamp_t timestamp)
{
...
void ep_execution_checkpoint_free(EventPipeExecutionCheckpoint *execution_checkpoint)
{
...
static bool enabled(void)
{
    ...
    return (
        ep_volatile_load_eventpipe_state_without_barrier() >= EP_STATE_INITIALIZED &&
...
static bool is_session_id_in_collection(EventPipeSessionID session_id)
{
    ...
    const uint8_t *related_activity_id)
{
    ...
    write_event_2(thread, ep_event, payload, activity_id, related_activity_id, NULL, NULL);
}
...
static const ep_char8_t *get_next_config_value(const ep_char8_t *data, const ep_char8_t **start, const ep_char8_t **end)
{
    EP_ASSERT(data != NULL);
...
static bool session_requested_sampling(EventPipeSession *session)
{
    ...
    return ep_rt_session_provider_list_find_by_name(
        ep_session_provider_list_get_providers_cref(ep_session_get_providers(session)),
...
#ifdef EP_CHECKED_BUILD
void ep_requires_lock_held(void)
{
    ep_rt_config_requires_lock_held();
}

void ep_requires_lock_not_held(void)
{
    ep_rt_config_requires_lock_not_held();
}
#endif

EventPipeSessionID ep_enable(
    const ep_char8_t *output_path,
    uint32_t circular_buffer_size_in_mb,
    const EventPipeProviderConfiguration *providers,
    uint32_t providers_len,
...
EventPipeSessionID ep_enable_2(
    const ep_char8_t *output_path,
    uint32_t circular_buffer_size_in_mb,
    ...
    void *callback_additional_data)
{
    ...
    if (!providers_config_to_parse || *providers_config_to_parse == '\0')
    {
        ...
        ep_provider_config_init(
            &providers[0],
            ep_rt_utf8_string_dup(ep_config_get_public_provider_name_utf8()),
            0x4c14fccbd,
            EP_EVENT_LEVEL_VERBOSE,
            NULL);
        ep_provider_config_init(
            &providers[1],
            ep_rt_utf8_string_dup(ep_config_get_private_provider_name_utf8()),
            0x4002000b,
            EP_EVENT_LEVEL_VERBOSE,
            NULL);
        ep_provider_config_init(
            &providers[2],
            ep_rt_utf8_string_dup(ep_config_get_sample_profiler_provider_name_utf8()),
            0x0,
            EP_EVENT_LEVEL_VERBOSE,
            NULL);
    }
...
void ep_disable(EventPipeSessionID id)
{
    ep_requires_lock_not_held();
...
EventPipeSession *ep_get_session(EventPipeSessionID session_id)
{
    ep_requires_lock_not_held();
...
bool ep_is_session_enabled(EventPipeSessionID session_id)
{
...
void ep_start_streaming(EventPipeSessionID session_id)
{
    ep_requires_lock_not_held();
...
bool ep_enabled(void)
{
    return (ep_volatile_load_eventpipe_state() >= EP_STATE_INITIALIZED && ep_volatile_load_number_of_sessions() > 0);
}

EventPipeProvider *ep_create_provider(
    const ep_char8_t *provider_name,
    ...
    void *callback_data)
{
    ...
    EventPipeProviderCallbackData provider_callback_data;
    EventPipeProviderCallbackDataQueue *provider_callback_data_queue =
        ep_provider_callback_data_queue_init(&data_queue);

    EP_LOCK_ENTER(section1)
    provider = config_create_provider(
        ep_config_get(),
        provider_name,
        callback_func,
        callback_data_free_func,
        callback_data,
        provider_callback_data_queue);
...
void ep_delete_provider(EventPipeProvider *provider)
{
...
EventPipeProvider *ep_get_provider(const ep_char8_t *provider_name)
{
    ep_return_null_if_nok(provider_name != NULL);
...
bool ep_add_provider_to_session(EventPipeSessionProvider *provider, EventPipeSession *session)
{
...
void ep_finish_init(void)
{
    ...
    if (ep_volatile_load_eventpipe_state() == EP_STATE_INITIALIZED)
    {
        ep_rt_session_id_array_iterator_t deferred_session_ids_iterator =
            ep_rt_session_id_array_iterator_begin(&_ep_deferred_enable_session_ids);
    ...
    if (ep_volatile_load_eventpipe_state() == EP_STATE_INITIALIZED)
    {
        ep_rt_session_id_array_iterator_t deferred_disable_session_ids_iterator =
            ep_rt_session_id_array_iterator_begin(&_ep_deferred_disable_session_ids);
        while (!ep_rt_session_id_array_iterator_end(
            &_ep_deferred_disable_session_ids,
            &deferred_disable_session_ids_iterator))
        {
            EventPipeSessionID session_id =
                ep_rt_session_id_array_iterator_value(&deferred_disable_session_ids_iterator);
...
EventPipeEventMetadataEvent *ep_build_event_metadata_event(EventPipeEventInstance *event_instance, uint32_t metadata_id)
{
...
void ep_write_sample_profile_event(
    ...
    uint32_t event_data_len)
{
    ...
    write_event_2(sampling_thread, ep_event, event_payload, NULL, NULL, target_thread, stack);

    ep_event_payload_fini(event_payload);
}

EventPipeEventInstance *ep_get_next_event(EventPipeSessionID session_id)
{
    ep_requires_lock_not_held();
...
EventPipeWaitHandle ep_get_wait_handle(EventPipeSessionID session_id)
{
    EventPipeSession *const session = ep_get_session(session_id);
    return session ? ep_rt_wait_event_get_wait_handle(ep_session_get_wait_event(session)) : 0;
}

bool ep_add_rundown_execution_checkpoint(const ep_char8_t *name, ep_timestamp_t timestamp)
{
    ep_requires_lock_not_held();
    ...
    EP_LOCK_ENTER(section1)
    ep_raise_error_if_nok_holding_lock(
        ep_rt_execution_checkpoint_array_append(&_ep_rundown_execution_checkpoints, exec_checkpoint),
        section1);
...
bool ep_provider_callback_data_queue_enqueue(
    EventPipeProviderCallbackDataQueue *provider_callback_data_queue,
    EventPipeProviderCallbackData *provider_callback_data)
{
    EP_ASSERT(provider_callback_data_queue != NULL);
    EventPipeProviderCallbackData *provider_callback_data_move =
        ep_provider_callback_data_alloc_move(provider_callback_data);
    ep_raise_error_if_nok(provider_callback_data_move != NULL);
    ep_raise_error_if_nok(ep_rt_provider_callback_data_queue_push_tail(
        ep_provider_callback_data_queue_get_queue_ref(provider_callback_data_queue),
        provider_callback_data_move));
...
bool ep_provider_callback_data_queue_try_dequeue(
    EventPipeProviderCallbackDataQueue *provider_callback_data_queue,
    EventPipeProviderCallbackData *provider_callback_data)
{
    EP_ASSERT(provider_callback_data_queue != NULL);

    ep_return_false_if_nok(!ep_rt_provider_callback_data_queue_is_empty(
        ep_provider_callback_data_queue_get_queue_ref(provider_callback_data_queue)));

    EventPipeProviderCallbackData *value = NULL;
    ep_raise_error_if_nok(ep_rt_provider_callback_data_queue_pop_head(
        ep_provider_callback_data_queue_get_queue_ref(provider_callback_data_queue),
        &value));
...
void ep_ipc_stream_factory_callback_set(EventPipeIpcStreamFactorySuspendedPortsCallback suspended_ports_callback)
{
