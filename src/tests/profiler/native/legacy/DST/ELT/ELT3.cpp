...
public:
	...
	HRESULT Verify(IPrfCom * pPrfCom)
	{
		if(pPrfCom->m_FunctionEnter3 != 0)
		{
			if(IsWithinFudgeFactor<LONG>(pPrfCom->m_FunctionLeave3 + pPrfCom->m_FunctionTailCall3, pPrfCom->m_FunctionEnter3, 0.9))
            {
				return S_OK;
            }
			else
		...
		if(pPrfCom->m_FunctionEnter3WithInfo != 0)
		{
			if(IsWithinFudgeFactor<LONG>(pPrfCom->m_FunctionLeave3WithInfo + pPrfCom->m_FunctionTailCall3WithInfo, pPrfCom->m_FunctionEnter3WithInfo, 0.9))
            {
				return S_OK;
            }
			else
...
void FastPathELT3_Initialize(IPrfCom *pPrfCom, PMODULEMETHODTABLE pModuleMethodTable)
{
	...
	// dll which want to SetFunctionIDMapper2. A better way would be to do this in the TestProfiler as it is being done now
...
void SlowPathELT2_Initialize(IPrfCom *pPrfCom, PMODULEMETHODTABLE pModuleMethodTable)
{
	...
	pModuleMethodTable->VERIFY = (FC_VERIFY) &FastPathELT3_Verify;
	pModuleMethodTable->FUNCTIONENTER2 =  &EnterLeaveTail3::FunctionEnter2Wrapper;
