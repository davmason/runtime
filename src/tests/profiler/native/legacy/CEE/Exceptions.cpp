...
#if defined(_ARM_) && defined(WINDOWS)
#include <process.h>
#endif
...
#define MUST_PASS(call)                         \
    do {                                          \
        HRESULT __hr = (call);                                          \
        if( FAILED(__hr) ) {                                            \
            FAILURE(L"call '" << STRINGIFY(call) << L"' failed with HR=" << __hr); \
...
#define REPORT(method) DISPLAY(L"(TID: " << HEX(threadId_) << L") CALLBACK " << STRINGIFY(method));

#define REPORT1(method,num) {                                   \
        WCHAR_STR( bh );                                        \
        MUST_PASS( pPrfCom->GetFunctionIDName(num, bh));        \
        DISPLAY(L"(TID: " << HEX(threadId_) << L") CALLBACK " << STRINGIFY(method) << L" (" << HEX(num) << L" - " << bh << L")"); \
    }    

void ExceptionsOnThread::register_thread(ThreadID threadId_)
{
    REPORT(register_thread);
    lock_guard<mutex> lock(g_threadLock);
...
#define VALUE(value) {ExceptionInFlight:: value, STRINGIFY(value)}
...
#undef VALUE
    ...
    {
        txt+=L"<invalid value> ";
        BufferHolder<wchar_t> bh;
        if( swprintf(bh.buffer(), bh.size(), L"%x", exceptionState) < 0 )
            txt+=bh.buffer();
    }
    return txt;
}
