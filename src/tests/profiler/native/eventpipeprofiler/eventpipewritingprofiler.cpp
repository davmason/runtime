...
HRESULT EventPipeWritingProfiler::Initialize(IUnknown* pICorProfilerInfoUnk)
{
    ...
    if (FAILED(hr = _pCorProfilerInfo12->EventPipeCreateProvider(U("MySuperAwesomeEventPipeProvider"), &_provider)))
    {
        _failures++;
        LogFailure(U("Could not create EventPipe provider hr={HR}"), hr);
        return hr;
    }

    // Create a param descriptor for every type
    COR_PRF_EVENTPIPE_PARAM_DESC allTypesParams[] = {
        { COR_PRF_EVENTPIPE_BOOLEAN,  0, U("Boolean") },
        { COR_PRF_EVENTPIPE_CHAR,     0, U("Char") },
        { COR_PRF_EVENTPIPE_SBYTE,    0, U("SByte") },
        { COR_PRF_EVENTPIPE_BYTE,     0, U("Byte") },
        { COR_PRF_EVENTPIPE_INT16,    0, U("Int16") },
        { COR_PRF_EVENTPIPE_UINT16,   0, U("UInt16") },
        { COR_PRF_EVENTPIPE_INT32,    0, U("Int32") },
        { COR_PRF_EVENTPIPE_UINT32,   0, U("UInt32") },
        { COR_PRF_EVENTPIPE_INT64,    0, U("Int64") },
        { COR_PRF_EVENTPIPE_UINT64,   0, U("UInt64") },
        { COR_PRF_EVENTPIPE_SINGLE,   0, U("Single") },
        { COR_PRF_EVENTPIPE_DOUBLE,   0, U("Double") },
        { COR_PRF_EVENTPIPE_GUID,     0, U("Guid") },
        { COR_PRF_EVENTPIPE_STRING,   0, U("String") },
        { COR_PRF_EVENTPIPE_DATETIME, 0, U("DateTime") }
    };
    ...
    {
        _failures++;
        LogFailure(U("FAIL: could not create EventPipe event with all types hr={HR}"), hr);
        return hr;
    }

    COR_PRF_EVENTPIPE_PARAM_DESC arrayTypeParams[] = {
        { COR_PRF_EVENTPIPE_ARRAY, COR_PRF_EVENTPIPE_INT32,    U("IntArray")},
    };
    const size_t arrayTypeParamsCount = sizeof(arrayTypeParams) / sizeof(arrayTypeParams[0]);
    hr = _pCorProfilerInfo12->EventPipeDefineEvent(
            _provider,                       // Provider
            U("ArrayTypeEvent"),         // Name
            3,                               // ID
            0,                               // Keywords
            1,                               // Version
            COR_PRF_EVENTPIPE_LOGALWAYS,     // Level
    ...
    {
        _failures++;
        LogFailure(U("FAIL: could not create array type EventPipe event with hr={HR}"), hr);
        return hr;
    }
    ...
    {
        _failures++;
        LogFailure(U("FAIL: could not create EventPipe event with no types hr={HR}"), hr);
        return hr;
    }
    ...
    {
        _failures++;
        LogFailure(U("FAIL: could not create EventPipe event with simple types hr={HR}"), hr);
        return hr;
    }
...
HRESULT EventPipeWritingProfiler::Shutdown()
{
    ...
    else
    {
        ...
        LogFailure(U("EventPipe profiler test failed, check log for more info."));
    }
    fflush(stdout);
...
HRESULT EventPipeWritingProfiler::FunctionSeen(FunctionID functionID)
{
    ...
    if (functionName == U("TriggerMethod"))
    {
        ...
        // { COR_PRF_EVENTPIPE_STRING, U("String") }
        LPCWCH str = U("Hello, this is a string!");
        ...
        // { COR_PRF_EVENTPIPE_DATETIME, U("DateTime") }
        ...
        if (FAILED(hr))
        {
            LogFailure(U("FAIL: EventPipeWriteEvent failed for AllTypesEvent with hr={HR}"), hr);
            _failures++;
            return hr;
        }
        ...
        // { COR_PRF_EVENTPIPE_FLAG_ARRAY_TYPE, COR_PRF_EVENTPIPE_INT32, U("IntArray")}
        ...
        if (FAILED(hr))
        {
            LogFailure(U("FAIL: EventPipeWriteEvent failed for ArrayTypeEvent with hr={HR}"), hr);
            _failures++;
            return hr;
        }
        ...
        for (int i= 0; i < 10; ++i)
        {
            ...
            if (FAILED(hr))
            {
                LogFailure(U("FAIL: EventPipeWriteEvent failed for EmptyEvent with hr={HR}"), hr);
                _failures++;
                return hr;
            }
        }
        ...
        for (int32_t i32 = 0; i32 < 10000; ++i32)
        {
            ...
            if (FAILED(hr))
            {
                LogFailure(U("FAIL: EventPipeWriteEvent failed for SimpleEvent with hr={HR}"), hr);
                _failures++;
                return hr;
            }
        }
